...
class LimoFinalController:
    ...
    def image_cb(self, msg):
        ...
        # [장애물 감지 트리거] 라바콘 주행 중에는 감지 거리를 살짝 줄여서 오작동 방지
            ...
            if len(raw) > 0:
                ...
                if len(left_zone) > 0 and len(right_zone) > 0:
                    ...
                    if avg_left < avg_right * 0.8:
                        if best_angle > -0.1: best_angle = -0.7
    ...
    def detect_cone(self, img):
        h, w = img.shape[:2]
        # 시야 확장: 0.55 -> 0.4 (더 멀리 있는 콘 인식)
        roi = img[int(h * 0.4):, :]
        hsv = cv2.cvtColor(roi, cv2.COLOR_BGR2HSV)
        # 감도 확장: 색상 범위를 더 넓게 잡음
    ...
    def cone_control(self, img):
        ...
        if len(centers) >= 2:
            ...
            # 하나만 보이면 세이프티 오프셋(140px) 적용하여 충돌 방지
            cone_x = centers[0]
            if cone_x > (w // 2):
                mid = cone_x - 140 # 오른쪽 콘 피해서 왼쪽 조준
            else:
                mid = cone_x + 140 # 왼쪽 콘 피해서 오른쪽 조준
            self.current_lin = 0.10

        error = mid - (w // 2)
        self.current_ang = np.clip(-error / 140.0, -0.7, 0.7)
